...
from sg2vid.data.dataset import WebVid10M, Pexels, JointDataset, SurgicalDataset
...
from sg2vid.utils.util import save_videos_grid, save_videos_grid_from_paths
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    elif train_data['dataset'] == "surgical":
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if class_label_condition_mode:
                    class_labels = batch['graph_embedding'].to(latents.device)
                ...
                if unet_additional_kwargs["first_frame_condition_mode"] != "none" and not class_label_condition_mode:
                    ...
                    model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, first_frame_latents=first_frame_latents, frame_stride=frame_stride, class_labels=class_labels).sample
                    loss = F.mse_loss(model_pred.float(), target.float()[:, :, 1:, :, :], reduction="mean")            
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process and global_step % validation_steps == 0:
                    ...
                    if class_label_condition_mode:
                        validation_graph_embs = train_dataset.validation_graph_embs

                    for idx, (prompt, first_frame_path, graph_emb) in enumerate(zip(prompts, first_frame_paths, validation_graph_embs)):
